...
export function normalizeForFuzzyMatch(text: string): string {
	return (
		text
			.normalize("NFKC")
			// Strip trailing whitespace per line. NOTE: intentionally NOT leading whitespace —
			// tried that (2026-07-05) to make matching indentation-insensitive, but the
			// replacement overlay (applyReplacementsPreservingUnchangedLines) misaligns offsets
			// when the base is de-indented and corrupts the indentation of single-line edits.
			// Fixing it safely needs re-indenting newText to the matched block's real indent — a
			// larger rework. See doc note; kept trailing-only for now (safe, self-correcting).
			.split("\n")
