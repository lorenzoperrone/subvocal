...
/**
 * The path shown in a tool breadcrumb. Prefer a project-root-relative form (e.g. `src/cart.ts`) —
 * the least noisy, most scannable representation, and what every good coding TUI shows. Fall back
 * to a `~`-home path only for files OUTSIDE the current project.
 */
export function displayToolPath(rawPath: string, cwd: string): string {
	const rel = getCwdRelativePath(resolvePath(rawPath, cwd), cwd);
	if (rel !== undefined && rel !== "") return rel.replace(/\\/g, "/");
...
export function renderToolPath(
	...
	return linkPath(theme.fg("accent", displayToolPath(value, cwd)), value, cwd);
}
